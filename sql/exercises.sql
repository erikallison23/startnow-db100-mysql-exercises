...
SELECT title,description,rating,length FROM film WHERE length > 179;
...
SELECT staff_id, first_name, last_name, address_id, picture, email, store_id, username, active,  last_update FROM staff WHERE password IS NOT NULL;



# 3h. Select all columns minus the password column from the staff table for rows that do not contain a password.
SELECT staff_id, first_name, last_name, address_id, picture, email, store_id, username, active,  last_update FROM staff WHERE password IS NULL;
...
SELECT title, length, description, rental_rate FROM film WHERE description like "%Database%" AND length >179;
...
SELECT title, description, special_features, length, rental_duration FROM film WHERE FIND_IN_SET("Behind the Scenes", special_features) AND length < 120 AND rental_duration BETWEEN 5 and 7 ORDER BY length DESC LIMIT 10; 
